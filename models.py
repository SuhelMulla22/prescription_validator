...
class PrescriptionAction(Action):
    ...
    drug_name: Optional[str] = Field(default=None, description="Name of the drug being flagged")
    ...
    rationale: Optional[str] = Field(default=None, description="Medical reasoning for this action")
...
class PrescriptionObservation(Observation):
    ...
    validation_results: List[Dict] = Field(
        default_factory=list, description="Results of automated safety checks performed so far"
    )

    current_issues: List[Dict] = Field(
        default_factory=list, description="Issues the agent has identified so far"
    )

    feedback: str = Field(
        default="", description="Human-readable feedback on the last action taken"
    )

    task_id: str = Field(
        default="easy", description="Current task difficulty: easy, medium, or hard"
    )
